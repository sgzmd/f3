...
if [ ! -d $active_dir ]; then
  ...
  if [ $? -ne 0 ]; then
    ...
    sed '/UNIQUE/d' flibusta_mysql_dump.sql > without_unique.sql

    # Applying MySQL -> sqlite3 converter script
    awk -f mysql2sqlite without_unique.sql >flibusta_sqlite_dump.sql
    if [ $? -ne 0 ]; then
